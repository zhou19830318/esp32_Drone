...
def servo_write_us(pwm, us):
    duty = int((us/20000)*1023)  # 20ms周期 (50Hz基准)
    pwm.duty(duty)

def channel_irq(pin):
    idx = channel_pins.index(pin.pin)
    now = micros()
...
def gyro_signals():
    ...
    RateYaw   = gz/65.5 - RateCalibrationYaw

    AccX = ax/4096 - AccXCalibration
    AccY = ay/4096 - AccYCalibration
    AccZ = az/4096 - AccZCalibration

    AngleRoll  = math.degrees(math.atan2(AccY, math.sqrt(AccX**2+AccZ**2)))
    AnglePitch = -math.degrees(math.atan2(AccX, math.sqrt(AccY**2+AccZ**2)))
...
def loop():
    ...
    while True:
        ...
        # 限幅
        MotorInput1 = min(max(MotorInput1,ThrottleIdle),1999)
        MotorInput2 = min(max(MotorInput2,ThrottleIdle),1999)
        MotorInput3 = min(max(MotorInput3,ThrottleIdle),1999)
        MotorInput4 = min(max(MotorInput4,ThrottleIdle),1999)
